...
configure_env() {
    ...
    echo "  - AUGMENT_DB_DIR (資料目錄路徑，可選)"
    echo "  - GLM_API_KEY (GLM-4.7 原廠 API Key，從 z.ai 獲取)"
    echo "  - MINIMAX_API_KEY (MiniMax-M2.1 原廠 API Key，從 minimax.io 獲取)"
    echo "  - REQUESTY_API_KEY (可選，用於備用路由)"
    echo ""
...
show_next_steps() {
    ...
    echo "     --env GLM_API_KEY=\"your-glm-api-key\" \\"
    echo "     --env MINIMAX_API_KEY=\"your-minimax-api-key\" \\"
